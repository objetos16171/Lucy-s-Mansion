     ...
     public void act() 
    {
       ...
       if(!destruir)  //Si la variable sigue siendo falsa
       {
           if(!isAtEdge())  //Si no está al borde
           {
               ...
               {
                   Personaje jug = getOneIntersectingObject(Personaje.class); //Regresa un objeto intersectado
                   jug.setAumentoVida(-daño);  //Se modifica la vida
                   destruir = true;  //Cambia el valor de la variable boleana
                }
                else
                {
                    if(intersects(Uteleria.class))  //Si está intersectando a alguna utilería
                    {
                        Utileria util = getOneIntersectingObject(Utileria.class);   //Regresa un objeto intersectado
        ...
        {
            this.setImage(bom); //Coloca imagen "bom"
