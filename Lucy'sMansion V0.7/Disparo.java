    ...
    */
     ...
     public void act() 
    {
       ...
       if(!destruir)  //Si la variable sigue siendo falsa
       {
           ...
           {
               if(intersects(getOneIntersectingObject(Personaje.class)))  //Si está intersectando a algún personaje
               {
                   Personaje jug = (Personaje)getOneIntersectingObject(Personaje.class); //Regresa un objeto intersectado
                   jug.setSalud(-danio);  //Se modifica la vida
                   destruir = true;  //Cambia el valor de la variable boleana
                }
                else
                {
                    if(intersects(getOneIntersectingObject(Utileria.class)))  //Si está intersectando a alguna utilería
                    {
